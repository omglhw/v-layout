  ...
  >
    ...
    >
      <div
        ...
        :title="selectedStr"
        class="text-overflow"
        :style="{ width: '100%',  position: 'absolute', left: '16px' }"
      ...
      <span v-if="collapseTags && selected.length && tag">
        <el-tag
  ...
  computed: {
    ...
    selectedStr () {
      return Array.isArray(this.selected)
        ? this.selected.map(item => item.currentLabel).join(',')
        : '';
    },
  },
      ...
      if (!val) {
        ...
        if (this.filterable) {
          ...
          if (this.multiple) {
            ...
            if (!this.remote) {
              this.broadcast('ElCheckOption', 'queryChange', '');
              this.broadcast('ElCheckOptionGroup', 'queryChange');
            }
  ...
  methods: {
    ...
    handleQueryChange (val) {
      ...
      if (this.remote && typeof this.remoteMethod === 'function') {
        ...
        this.broadcast('ElCheckOption', 'queryChange', val);
        this.broadcast('ElCheckOptionGroup', 'queryChange');
      }
      if (
        this.defaultFirstOption &&
